# oh-my-claudedocs 글로벌 규칙

> 모든 프로젝트에 공통 적용되는 규칙. 프로젝트 `.claude/rules/` 에 배치된다.
> 상세 참조: `omcd-ref.md` (거버넌스 정의, SRS/PRD 선택 기준, 갱신 조건, IEEE 매핑)
...
상위 문서 갱신 시 하위/연관 문서의 갱신 필요성을 사용자에게 알린다. 자동 갱신하지 않고 사용자 확인 후 진행한다. 개별 갱신 조건은 `omcd-ref.md` 참조.
...
가능한 모든 작업은 직접 수행하지 않고, **스크립트를 먼저 작성하여 실행**한다.

- 스크립트는 `claude_temp/`에 저장한다 (프로젝트 루트 기준)
- 파일 이름은 작업 내용을 반영한다 (예: `fix-lint-errors.mjs`, `migrate-schema.sh`)
- 실행: `node claude_temp/xxx.mjs` 또는 `bash claude_temp/xxx.sh`
- 스크립트 작성 → 실행 → 결과 확인 순서를 따른다
- 단순 명령 1줄(예: `git status`, `npm test`)은 예외로 직접 실행 가능

이유: 재현 가능성, 감사 추적, 오류 감소. 스크립트는 사용자가 검토하거나 재실행할 수 있다.

## 11. push 전 자체 리뷰

`git push` 전에 반드시 `node claude_temp/review-before-push.mjs`를 실행한다.

- 변경 파일 분류, 커밋 형식 검증, PR 상태 확인, JS 구문 검증을 자동 수행
- 리포트에서 `✗` 항목이 있으면 수정 후 push
- 리뷰 결과를 사용자에게 보여준 뒤 push 진행
...
- Rules 파일(`omcd.md`)도 동일하게 간결하게 유지한다.
