# oh-my-claudedocs - 개발 자동화 시스템 설계 명세
...
| 1 | `CLAUDE.md` 템플릿 → 프로젝트 루트에 복사 (OMC 섹션과 oh-my-claudedocs 섹션 분리) |
...
OMC 플러그인 훅(`hooks.json`)은 플러그인 소유로 사용자 확장 불가. oh-my-claudedocs 훅은 `settings.json`의 사용자 훅으로 등록하며, 두 시스템은 병렬 공존한다 (플러그인 훅 → 사용자 훅 순차 실행).
    ...
    "SessionStart": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "node ~/.claude/hooks/omcd/session-start.mjs",
          "timeout": 5
        }]
      }
    ],
    "PreToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "node ~/.claude/hooks/omcd/pre-commit-check.mjs",
          "timeout": 10
        }]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "node ~/.claude/hooks/omcd/post-save-mmd.mjs",
          "timeout": 15
        }]
      }
    ]
...
스킬은 3개 계층으로 구분된다: OMC 내장 → 외부 (find-skills 탐색/설치) → 커스텀 (omcd 제공).
...
## 6. Rules (글로벌 `~/.claude/rules/omcd.md`)
...
> **OMC 공존**: CLAUDE.md는 OMC도 읽고 참조한다. oh-my-claudedocs 설정과 OMC 가이드가 충돌하지 않도록 섹션을 명확히 분리한다.
...
| 분류 | 문서 | 설명 | OMC 지원 | oh-my-claudedocs 보완 |
|------|------|------|----------|----------------|
...
> **OMC 지원 범례**: O = OMC 자동 생성 flow 존재, △ = 에이전트가 작성 가능하나 자동 flow 없음, X = OMC 미지원. **oh-my-claudedocs 보완** 컬럼은 커스텀 스킬/훅/템플릿으로 갭을 메우는 방식을 표시한다.
