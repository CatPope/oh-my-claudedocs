...
> 상세 참조: `~/.claude/docs/docs-omc-ref.md` (거버넌스 정의, SRS/PRD 선택 기준, 갱신 조건, IEEE 매핑) — 필요 시 Read로 접근
...
상위 문서 갱신 시 하위/연관 문서의 갱신 필요성을 사용자에게 알린다. 자동 갱신하지 않고 사용자 확인 후 진행한다. 개별 갱신 조건은 `~/.claude/docs/docs-omc-ref.md` 참조.

## 8. 문서 관리 정책

- 문서 작성과 검토는 항상 세트로 진행한다. 문서 수정 후 `/docs-reviewer`로 검토
...
> **적용 범위: `docs/dev/*.md` 파일에만 적용한다.** CLAUDE.md, README.md 등 프로젝트 루트 파일에는 적용하지 않는다.

`docs/dev/` 내 모든 문서는 첫 15줄에 제목과 대분류 목차만 포함한다 (15줄째 = `---`).
...
- 현재 작업에 필요한 MCP만 활성화한다 (프런트엔드 → Playwright, 백엔드 → Database 등)
- 프로젝트별 `.claude/settings.local.json`에서 불필요한 MCP를 비활성화한다
- 정기적으로 MCP 목록을 점검하여 미사용 항목을 제거한다
